...
def get_human_readable_duration(duration_raw_s : str):
    ...
    if (minutes > 0):
        formatted_str += (", " if (formatted_str != "") else "") + "{} ".format(minutes) + "minute" + ("s" if minutes > 1 else "")
    ...
    if (seconds > 0) or ((seconds == 0) and (formatted_str == "")):
        formatted_str += (", " if (formatted_str != "") else "") + "{} ".format(seconds) + "second" + ("s" if seconds != 1 else "")

    return formatted_str
    #return (f'{hours:d} hours, {minutes:d} minutes, {seconds:d} seconds')
...
def create_call_log(calls_xml_dir):
    ...
    for filename in os.listdir(calls_xml_dir):
        ...
        if filename.endswith(".xml") and filename.startswith("calls"):
            xml_tree = xml.etree.ElementTree.parse(os.path.join(calls_xml_dir, filename))
            ...
            for call_entry_xml in xml_tree.findall(".//call"):
                ...
                if not call_entry_xml.attrib['date'] in call_timestamps:
                    ...
                    if call_type != "Missed":
                        call_entry_obj["Call duration (s)"] = call_duration_raw # the 'raw' value
                        call_entry_obj["Call duration"] = get_human_readable_duration(call_duration_raw)
        ...
        print(f'[DEBUG] Finished processing file {filename} .. now at {num_calls} calls total')
